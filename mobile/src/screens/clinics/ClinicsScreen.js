      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ListItemCard
            ...
            badge={item.is_active ? 'Ativa' : 'Inativa'}
            badgeColor={item.is_active ? colors.success : colors.textMuted}
            onPress={() => navigation.navigate('ClinicaForm', { item })}
          />
        )}
        ListEmptyComponent={<EmptyState message="Nenhuma clínica cadastrada" />}
        contentContainerStyle={{ paddingBottom: 90 }}
      />
      <TouchableOpacity
        style={styles.fab}
        onPress={() => navigation.navigate('ClinicaForm', {})}
        activeOpacity={0.85}
      >
