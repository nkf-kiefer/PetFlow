      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ListItemCard
            title={item.name}
            subtitle={item.email}
            badge={ROLE_LABEL[item.role] ?? item.role}
            badgeColor={item.is_active ? colors.primary : colors.textMuted}
            onPress={() => navigation.navigate('FuncionarioForm', { item })}
          />
        )}
        ListEmptyComponent={<EmptyState message="Nenhum funcionário cadastrado" />}
